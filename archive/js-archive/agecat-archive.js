...
function drawLine(svg, key, plotdata, index, stop){
	...
	if (!web){
		...
		labels
			...
			let datatips = svg.select(".series"+index).selectAll(".datatip")
			    .data(linedata);

			//get bin intervals
			let binCoords = plotdata.data.bins.map( bin => {
				return [bin.x, bin.x+bin.width] 
			});
			...
			datatips
			    ...
			    .text(function (d, i) {
				    return `Ages ${binCoords[i][0]}—${binCoords[i][1]}, ${Math.round(d.percent)}%`;
				})
...
function setupPlotLabels(key, plotdata){
	...
	if(typeof plotdata.labels !== 'undefined'){
		...
		plotdata.labels.forEach( (label, i) =>{
			...
			area.on('mousemove', function(event, d){
				// console.log(event, d, i);
				let x = plotdata.scales.x;
				// //get x coord of hover-over position;
				let xcoord = x.invert(d3.pointer(event)[0]);
				// get relevant BIN
				let thisBinIndex = 0;
				plotdata.data.bins.forEach( (bin, j)=>{
					if( xcoord >= bin.x && xcoord < bin.x+bin.width ){
						thisBinIndex = j;
					}
				});
				console.log('thisbin', thisBinIndex);

				// let tips = d3.select(`#${key}-group .series${i}`).selectAll('.datatip');
				let tips = plot.querySelector('.series'+i).querySelectorAll('.datatip');
				let tipElement = tips[thisBinIndex];

				if( !tipElement.classList.contains('activetip')){
					let openTip = document.querySelector('.activetip');
					if(openTip){
						openTip.classList.remove('activetip');
					}					
					tipElement.classList.add('activetip');
				}
...
}) (typeof exports === 'undefined'? this['agecat']={}: exports);